...
import {selectBasket, addItems, delItems} from '../redux/slices/basketSlice'
import {selectFavourite, addItemsFavourite, delItemsFavourite} from '../redux/slices/favouriteSlice'
...
const Cart:React.FC<itemsType> = ({ id, title, price, imageURL}) => {
      const { basketItems } = useSelector(selectBasket);    
      const { favouriteItems } = useSelector(selectFavourite)
      ...
      const addToCartFavourite = () => {
            if (location.pathname === '/') {
                  const obj: productType = {
                        id: `${favouriteItems.length + 1}`,
                        productId: id,
                        title,
                        price,
                        imageURL
                  }
                  if (!favouriteItems.some((item) => Number(item.productId) === Number(obj.productId))) {
                        dispatch(addItemsFavourite(obj))
                  } else {
                        favouriteItems.filter((item) => {
                              if (Number(item.productId) === Number(obj.productId)) {
                                    dispatch(delItemsFavourite(item.id))
                              }
                        });
                  }
            }
      ...
      const getIsAddedFavourite = (id: string) => {
            if (location.pathname === '/favourites') {
                  return favouriteItems.some((item) => Number(item.id) === Number(id))      
            } else {
                  return favouriteItems.some((item) => Number(item.productId) === Number(id))
            }
      }
      
      return (
            <div className="section-bottom__column">
                  <div className="section-bottom__item">
                        <div className="section-bottom__like" onClick={() => {addToCartFavourite()}}>
                        {getIsAddedFavourite(id) ? 
                        ...
                        <div className="section-bottom__imageURL">
                              <img width={110} height={110} alt="sneakers" src={imageURL} />
                        </div>
                        <div className="section-bottom__title">
                              {title}
                        </div>
                        <div className="section-bottom__block">
                              <div className="section-bottom__text">
                                    <h2>Цена:</h2>
                                    <p>{price} руб.</p>
                              </div>
